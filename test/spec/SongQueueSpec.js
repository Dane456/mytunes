describe('SongQueueCollection', function() {
  ...
  describe('when a song ends', function() {
    it('removes the song from the queue', function() {
      var songQueue = new SongQueueCollection([songData1, songData2]);
  ...
  describe('when a song is dequeued', function() {
    it('removes the song', function() {
      removeSpy = sinon.spy(SongQueueCollection.prototype, 'remove');
      var songQueue = new SongQueueCollection(songData1);
      songQueue.at(0).dequeue();
      expect(removeSpy).to.have.been.called;
      SongQueueCollection.prototype.remove.restore();
    });
  });
